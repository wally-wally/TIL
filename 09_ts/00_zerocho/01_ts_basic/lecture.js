...
var num; // any형이 되지 않도록 type을 명시하여 막아준다.
// let num: Number; // type 선언시 맨 처음 알파벳을 대문자로 쓰지 말자!(전부 소문자로!)
num = 3;
// (1-2) type 선언 방법
// let str: string = num; // 이건 에러
// let str: string = num.toString(); // 방법(1)
var str = String(num); // 방법(2)
// (2) 배열 선언 방법
var arr = [1, 2, 3]; // 배열 표기법(1) => number나 string일 때 주로 이 방법
// let arr: Array<number> = [1, 2, 3]; // 배열 표기법(2) => 제네릭 쓸 때 주로 이 방법
// (3) 튜플
var newArr = [true, 2, '3']; // 여러 type이 있는 경우
// let newArr: (string | number | boolean)[] = [true, 2, {}]; // 배열 요소에 다른 type(여기선 객체로 넣어봄)을 넣으면 error
var newArray = [true, 2, '3']; // 더 엄격하게 배열 선언(배열 길이가 3이고 type 순서가 boolean, number, string인 경우)
// 위와 같이 특별한 경우를 tuple 이라고 한다.
// 단, tuple에 push하는 행위는 막지 못한다.
// 여기서 더 엄격하게 한다면?
var newArray2 = [true, 2, '3']; // 두 번째 요소를 숫자 2로 고정
var originalArr = [true, 2, '3']; // 사실 이렇게만 써도 originalArr에 마우스를 올리면 type이 어떻게 선언되는지 알아서 알려준다.
// (4) 상수, 객체(객체는 object라고 type을 선언하지 않는다. 워낙 형태가 다양...)
var fixArr = [true, 2, '3']; // as const를 붙여주면 fixArr 내용을 바꿀 수 없다.(readonly)
var obj = { a: 'k' };
...
var secondObj = { a: 'k' };
// secondObj.a = 'c'; // 불가능
var thridObj = { a: 'k', b: 1 };
// thridObj.a = 3; // string을 넣지 않았으므로 error
var fourthObj = { a: 'k' }; // b가 나중에 생길지 안 생길지 모르는 경우 ?를 붙여준다.
// (5) enum
var Color;
(function (Color) {
...
hello.substr(1, 2); // 강제 type 변환 방법 - 1
// 타입이 겹치는 경우(ex. d.ts에서 interface에서 부모-자식 관계) as unknown을 쓰지 않아도 되지만 위와 같은 경우는 number와 string 명확히 다르므로 as unknown을 써줘야 한다.
hello.substr(1, 2); // 강제 type 변환 방법 - 2
// 가급적이면 as unknown을 쓰지 않는 것이 좋지만 프로젝트를 하다 보면 어쩔 수 없이 써야하는 경우가 생기므로 알아두자.
var div = document.createElement('div'); // HTMLDivElement
var a = div; // 이럴 때는 타입이 겹치므로 as unknown을 쓰지 않아도 된다.(자식인 HTMLDivElement 타입은 부모인 HTMLElement를 상속 받음)
var b = div; // number와 HTMLDivElement는 겹치는 관계가 아예 없으므로 as unknown을 붙여서 강제로 타입 변환을 해줘야 한다.
