...
# 1. 재귀 함수를 이용해 만든 순열 이용
def perm(k, distance_value):
    global result, order
    if distance_value > result:
        return
    if k == customer_cnt + 1:
        distance_value += abs(position_list[-1][0] - position_list[order[-1]][0]) + abs(position_list[-1][1] - position_list[order[-1]][1])
        if distance_value <= result:
        ...
        for i in range(1, customer_cnt + 1):
            if not visited[i]:
                ...
                if len(order) == 1:
                    perm(k + 1, distance_value + abs(position_list[0][0] - position_list[order[0]][0]) + abs(position_list[0][1] - position_list[order[0]][1]))
                else:
                    perm(k + 1, distance_value + abs(position_list[order[-2]][0] - position_list[order[-1]][0]) + abs(position_list[order[-2]][1] - position_list[order[-1]][1]))                    
                visited[i] = 0
                order.pop()
